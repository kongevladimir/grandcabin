...
import { OwnerPriceCalendar } from "./OwnerPriceCalendar";
import { useCallback, useEffect, useRef, useState } from "react";
import { RetreatNav } from "@/components/ConceptPages";
import { useSiteLanguage } from "@/components/useSiteLanguage";
import { dateValue, nights, osloToday, rentalQuote, type Block, type Booking, type BookingStatus, type NightlyPrices } from "@/lib/booking/model";
...
type OwnerData = { bookings: VisibleBooking[]; blocks: Block[]; pendingEmails: number; preview: boolean; nightlyPrices: NightlyPrices; today: string };
...
export function OwnerInbox() {
  const [view, setView] = useState<"prices" | "enquiries">("prices");
  ...
  return <div className="concept-page retreat-page bk-page"><RetreatNav language={lang} setLanguage={setLang} /><main className="bk-main bk-owner"><div className="bk-owner-heading"><div><p className="bk-kicker">GRANDCABIN · {w("EIER", "OWNER")}</p><h1>{w("Administrasjon", "Administration")}</h1></div>{data && <button className="bk-secondary" onClick={async () => { await bookingApi("", { action: "logout" }); setData(null); setPassword(""); }}>{w("Logg ut", "Sign out")}</button>}</div>
    ...
    {!data ? loading ? <p role="status">{w("Laster inn …", "Loading …")}</p> : <form className="bk-panel bk-login" onSubmit={async e => { e.preventDefault(); await action({ action: "login", password }); setPassword(""); }}><h2>{w("Velkommen tilbake.", "Welcome back.")}</h2><p>{w("Logg inn for å svare gjester og administrere kalenderen.", "Sign in to reply to guests and manage the calendar.")}</p><label>{w("Eierpassord", "Owner password")}<input type="password" autoComplete="current-password" required value={password} onChange={e => setPassword(e.target.value)} /></label><button className="bk-primary" disabled={busy}>{w("Logg inn", "Sign in")} →</button></form> : <>
      ...
      <div className="bk-owner-tabs" aria-label={w("Administrasjon", "Administration")}><button type="button" className={view === "prices" ? "active" : ""} aria-pressed={view === "prices"} onClick={() => setView("prices")}>{w("Priskalender", "Price calendar")}</button><button type="button" className={view === "enquiries" ? "active" : ""} aria-pressed={view === "enquiries"} onClick={() => setView("enquiries")}>{w("Forespørsler & datoer", "Enquiries & dates")}</button></div>
      {view === "prices" ? <OwnerPriceCalendar prices={data.nightlyPrices} today={data.today} occupied={[...data.blocks, ...data.bookings.filter(b => b.status === "accepted")]} lang={lang} onSaved={reload} /> : <>
