...
import { CLEANING_FEE_NOK, DAILY_RATE_NOK, EXTRA_GUEST_RATE_NOK, INCLUDED_GUESTS, LINEN_TOWELS_RATE_NOK, dateValue, nights, osloToday, overlaps, rentalQuote, validateStay, type Stay } from "@/lib/booking/model";
import { bookingApi, errorText, formatDay, money, priceSeasonLabel, words, type Lang } from "./shared";

type Availability = { ready: boolean; preview: boolean; unavailable: Stay[]; today: string };
...
export function BookingForm() {
  ...
  const quote = rentalQuote(stay, linenTowels, guests);
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const result = await bookingApi<{ id: string; token: string }>("", { action: "enquire", ...stay, guests, linenTowels, name, email, phone, occasion, message, consent, language: lang, website, requestKey: submissionKey.current });
      router.push(`/booking/conversation/${result.id}#access=${result.token}`);
    } catch (e) { setError((e as Error).message); if ((e as Error).message === "unavailable") { setStep(0); await load(); } }
